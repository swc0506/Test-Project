...
namespace U3DExtends
{
    public class SceneEditor
    {
        ...
        static bool HandleDragAsset(SceneView sceneView, Object handleObj)
        {
            ...
            {
                //>>>>>>>>>>>>>>ZM>>>>>>>>>>>>>>>>>
                if (!handleObj.name.Contains("Window") && UIEditorHelper.GetUITestRootNode() == null)
                {
                    Debug.Log("请先创建界面模板！");
                    return false;
                }
                GameObject new_obj = GameObject.Instantiate(handleObj) as GameObject;
                if (new_obj.name.Contains("TempWindow"))
                {
                    GameObject uiRoot = GameObject.Find("UIRoot");
                    if (uiRoot == null)
                    {
                        Debug.LogError("场景中不存在UIRoot节点，请自行进行创建！");
                        return false;
                    }
                    new_obj.transform.SetParent(uiRoot.transform);
                    new_obj.name = new_obj.name.Replace("(Clone)", "");
                    Transform uiCameraTsf = GameObject.Find("UICamera").transform;
                    if (uiCameraTsf == null)
                    {
                        Debug.LogError("UIRoot节点下没有UICamera，请自行进行创建！");
                        return false;
                    }
                    new_obj.GetComponent<Canvas>().worldCamera = uiCameraTsf.GetComponent<Camera>();
                    return true;
                }
                if (handleObj.name.Contains("Window"))
                {
                    return false;  
                }
                if (new_obj != null)
