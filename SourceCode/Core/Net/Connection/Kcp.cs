...
namespace Core.Net
{
    public class Kcp
    {
        ...
        internal class Segment
        {
            ...
            internal UInt32 rto;
            internal UInt32 faskack;
            internal UInt32 xmit;
            internal byte[] data;

            internal Segment(int size = 0)
            {
                data = size > 0 ? new byte[size] : null;
            }

            internal void Encode(byte[] ptr, ref int offset)
            {
                UInt32 len = null != data ? (UInt32)data.Length : 0;
        ...
        private Segment tempSeg = new Segment();
        ...
        void Flush()
        {
            ...
            tempSeg.Clear();
            tempSeg.conv = conv_;
            tempSeg.cmd = IKCP_CMD_ACK;
            tempSeg.wnd = (UInt32)WndUnused();
            tempSeg.una = rcv_nxt_;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                ACKGet(i, ref tempSeg.sn, ref tempSeg.ts);
                tempSeg.Encode(buffer_, ref offset);
            }
