...
namespace U3DExtends
{
    public static class UIEditorHelper
    {
        ...
        public static Transform GetRootLayout(Transform trans)
        {
            ...
            //    }
            //}
            result = trans.Find("UIContent");
            if (result == null)
            {
                Debug.LogError("模板内容界面名称无法正确对应请检查模板节点下是否有 UIContent节点！ ");
            }
            return result;
        }
        ...
        {
            GameObject testUI = null;
            //for (int i = 0; i < 5; i++)
            //{
            //    testUI = GameObject.Find(Configure.UITestNodeName + i);
            //    if (testUI != null)
            //        break;
            //}
            //if (testUI == null)
            //{
            //    testUI = GameObject.Find(Configure.UITestNodeName);
            //}
            int objIndex = -1;
            object[] objects = GameObject.FindObjectsOfType<GameObject>();
            for (int i = 0; i < objects.Length; i++)
            {
                testUI = objects[i] as GameObject;
                if (testUI.name.Contains("Window"))
                {
                    objIndex = i;
                }
            }
            //if (testUI==null)
            //{
            //  testUI=  AssetDatabase.LoadAssetAtPath<GameObject>(Configure.PfbModelAssetsPath+"/"+ Configure.UITestNodeName+"2.prefab");
            //}
            if (objIndex==-1)
            {
                return null;
            }
            testUI = objects[objIndex] as GameObject;
            return testUI;
        }
