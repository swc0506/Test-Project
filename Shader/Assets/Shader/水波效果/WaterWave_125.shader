    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                //将切线空间下的法线转换到世界空间下
                float3 worldNormal = float3(dot(i.T2W0.xyz, bump), dot(i.T2W1.xyz, bump), dot(i.T2W2.xyz, bump));
                
                //计算反射向量
                fixed3 worldReflect = reflect(-viewDir, worldNormal);
                fixed4 reflColor = texCUBE(_Cube, worldReflect) * tex2D(_MainTex, i.uv * speed); //反射光线采样颜色纹理
                ...
                float4 c = reflColor * fresnel + grabColor * (1 - fresnel);
